...
function updateEnabledState(enabled) {
  ...
  statusText.textContent = `Auto-archiving is ${enabled ? 'enabled' : 'disabled'}`;
}
...
function renderHistory(history) {
  ...
  if (history.length === 0) {
    historyList.innerHTML = '<div class="empty-history">No archives yet</div>';
    return;
  }
...
async function initPopup() {
  try {
    ...
    historyList.innerHTML = '<div class="empty-history">Error loading data</div>';
  }
}
